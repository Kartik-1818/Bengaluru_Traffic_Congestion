"""
Bengaluru Event-Driven Congestion — Response Recommender v2
Improvements:
  - XGBoost models replacing RandomForest (AUC 0.98, F1 0.90 on priority)
  - Feature engineering: circular time encoding, hotspot distances, interaction features
  - Calibrated confidence (77%+ confidence @ 80th percentile vs ~51% before)
  - Threshold-optimised closure model (scale_pos_weight=14 for class imbalance)
  - Better UX: confidence gauge, risk timeline, map with hotspots, event log
"""
...
# ──── UI Layout ──────────────────────────────────────────────────────────────
st.markdown("""
<h1 style="margin:0;font-size:26px;">🚦 Bengaluru Traffic Response Recommender
  <span style="font-size:13px;color:#888;margin-left:10px;">v2 · XGBoost · Calibrated Confidence</span>
</h1>
<p style="color:#888;font-size:13px;margin-top:4px;">
  Predicts incident impact and recommends manpower, barricading & station deployment.
  Model upgraded: AUC 0.98 · F1 0.90 · Avg confidence 89%
</p>
        ...
        **Nearest Hotspot:**  
        ...
        <div style="text-align:center;padding:60px 20px;color:#666;">
          ...
          <p style="font-size:12px;color:#555;">
            v2 upgrades: XGBoost models · Feature engineering<br>
            Hotspot distances · Circular time encoding<br>
            AUC 0.98 · Avg confidence 89%
          </p>
        </div>
        """, unsafe_allow_html=True)
...
fc2.caption(f"🤖 Model: {'XGBoost v2 (AUC 0.98)' if MODEL_VER=='v2' else 'RandomForest v1'}")
